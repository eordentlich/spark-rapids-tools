...
parser.add_argument("--holdout", type=str, help="holdout snippets", required=True)
...
# CPU log parsing
cpu_dirs = glob.glob(cpu_dir)
gpu_dirs = glob.glob(gpu_dir)
print(f"number of cpu dirs {len(cpu_dirs)}")
print(f"number of gpu dirs {len(gpu_dirs)}")
print(f"number of holdout strings {len(holdout)}")
for app in cpu_dirs:
    if any([h in app for h in holdout]):
        print(f"skipping {app}")
        continue
    # - figure out query from application_info.csv
    app_info = pd.read_csv(app + "/sql_level_aggregated_task_metrics.csv")
    app_name = app_info.loc[0]["appID"]
    ...
    mapping_info = pd.read_csv(app + "/wholestagecodegen_mapping.csv")
    ...
    cpu_sql_info = pd.read_csv(app + "/sql_plan_metrics_for_application.csv")
    ...
    cpu_stage_info = pd.read_csv(app + "/sql_to_stage_information.csv")
...
for app in gpu_dirs:
    ...
    gpu_stage_info = pd.read_csv(app + "/sql_to_stage_information.csv")
